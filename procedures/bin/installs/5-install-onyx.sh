...
#   2-install-jdk.sh has been run to the Java sdk.
...
#=========================================================================== 
echo "About to install Onyx within the Tomcat web server..."
echo ""
...
#
# Remove old attempt if it exists...
...
#
# Unzip Onyx war file into temp directory...
print_message "" $LOG_FILE
print_message "Unzipping  Onyx war file into temp directory..." $LOG_FILE
unzip -o $WORK_DIR/$ACQUISITIONS_DIRECTORY/$( basename $ONYX_DOWNLOAD_PATH ) -d $WORK_DIR/temp >>$LOG_FILE 2>>$LOG_FILE
exit_if_bad $? "Failed to unzip Onyx war file to $WORK_DIR/temp" $LOG_FILE

#
# Overwrite config files...
print_message "" $LOG_FILE
print_message "Overwriting Onyx config files in expanded war" $LOG_FILE
cp $ONYX_INSTALL_PROCS_HOME/config/onyx/* $WORK_DIR/temp/WEB-INF/config/
exit_if_bad $? "Could not overwrite config files in $WORK_DIR/temp/WEBINF/config" $LOG_FILE
...
cd $WORK_DIR/temp
zip -r $WORK_DIR/$( basename $ONYX_DOWNLOAD_PATH ) * >>$LOG_FILE 2>>$LOG_FILE
...
TOMCAT_INSTALL_DIRECTORY_NAME=$( basename $TOMCAT_DOWNLOAD_PATH .zip )
cp $WORK_DIR/$( basename $ONYX_DOWNLOAD_PATH) $ONYX_INSTALL_DIRECTORY/$TOMCAT_INSTALL_DIRECTORY_NAME/webapps/$ONYX_WAR
exit_if_bad $? "Could not copy Onyx war into  $ONYX_INSTALL_DIRECTORY/$TOMCAT_INSTALL_DIRECTORY_NAME/webapps" $LOG_FILE
...
#=========================================================================
print_message "Onyx installation completed." $LOG_FILE
